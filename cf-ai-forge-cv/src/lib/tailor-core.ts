...
export interface TailorResult {
  ...
  reasoning: { section: string; change: string; why: string; coachingNote?: string }[];
}
...
export function buildTailorUserMessage(
  ...
  const jd = jobDescription.length > MAX_JD_CHARS
    ? jobDescription.slice(0, MAX_JD_CHARS)
    : jobDescription;

  const resumeRaw = JSON.stringify(resume, null, 2);
  const resumeStr = resumeRaw.length > MAX_RESUME_CHARS
    ? resumeRaw.slice(0, MAX_RESUME_CHARS) + "\n...[truncated for length]"
    : resumeRaw;

  const masterRaw = masterProfile ? JSON.stringify(masterProfile, null, 2) : null;
  const masterSection = masterRaw && masterRaw !== resumeRaw
    ? `\n\nADDITIONAL MASTER PROFILE CONTEXT (same candidate — pull any entries not already in the resume above):\n${masterRaw.length > MAX_MASTER_CHARS ? masterRaw.slice(0, MAX_MASTER_CHARS) + "\n...[truncated for length]" : masterRaw}`
    : "";

  return `CANDIDATE FULL CAREER HISTORY (master profile — select and tailor the most relevant entries for this role):\n${resumeStr}\n\nJOB DESCRIPTION:\n${jd}${masterSection}`;
}

export function parseTailorResponse(raw: string, resume: ResumeData): TailorResult {
  ...
  const parsed = JSON.parse(jsonStr) as TailorResult;
  ...
  // Strip errant "- " / "• " prefixes from skill strings, deduplicate across categories
  if (Array.isArray(parsed.tailored.skills)) {
    const seen = new Set<string>();
    parsed.tailored.skills = parsed.tailored.skills
      .map((cat) => ({
        ...cat,
        skills: (cat.skills ?? [])
